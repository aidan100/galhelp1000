...
function check($input_text)
{
    ...
    foreach ($lines as $line) {
        ...
        if (empty($line_array[0])) {
            $total_marks = "One of the module names is empty";
        } elseif (empty($line_array[1])){
            $total_marks = "One of the module marks is empty";    
        } elseif (($line_array[1] < 0) || ($line_array[1] > 100)) {
            $total_marks = "number must be between 0-100";
        } elseif (!is_numeric($line_array[1])) {
            $total_marks = "the grade must be numeric";
        } else {
        }
    }
    return $total_marks;
