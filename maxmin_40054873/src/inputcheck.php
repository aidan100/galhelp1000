...
function check($input_text)
{
    ...
    foreach ($lines as $line) {
        ...
        if (empty($line_array[0])) {
            $maxminModule = "One of the module names is empty";
        } elseif (empty($line_array[1])){
            $maxminModule = "One of the module marks is empty";
        } elseif (($line_array[1] < 0) || ($line_array[1] > 100)) {
            $maxminModule = "number must be between 0-100";
        } elseif (!is_numeric($line_array[1])) {
            $maxminModule = "the grade must be numeric";
        } else {
        }
    }
    return $maxminModule;
